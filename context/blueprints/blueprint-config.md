...
This blueprint covers the configuration discovery, merging, validation, and runtime query system for caveman-cli. The system must support both directory-tree traversal discovery (`.cave/config.md`, `AGENTS.md`, `CLAUDE.md`) and hierarchical merging with project-local overrides. Configuration must be validated against known schemas and provide introspection capabilities so users understand where each setting originates.

## Requirements

### R-001: Directory Traversal Configuration Discovery

**Description:** Configuration files must be discovered by walking the directory tree upward from the current working directory, with a defined search order and early termination strategy. Both `.cave/` directory and flat file naming (`AGENTS.md`, `CLAUDE.md`) must be supported.
...
**Description:** When configuration files exist at multiple directory levels, settings must merge using a documented precedence strategy: child directories override parent values for keys present in both.
...
**Description:** The configuration system must validate loaded settings against known keys and apply sensible defaults for missing configuration.
...
**Description:** The resolved configuration must be queryable at runtime, and users must be able to inspect which values came from which source (global, project-local, or default).
...
- Plugin-specific configuration (covered by blueprint-extension-core R2)

## Cross-References

- blueprint-fork-identity: Provides the host agent infrastructure and project structure
- blueprint-extension-core: Extension configuration system (R2) depends on this kit's discovery and merging logic
- AGENTS.md: Source of truth for agent conventions and project-local config naming
- CLAUDE.md: Reference material storage pattern and discovery

## Changelog

| Date | Change |
|------|--------|
| 2026-04-11 | Initial draft for config management domain |
